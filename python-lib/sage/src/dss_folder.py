...
def write_local_folder_output(self, project_handle, folder_name, path, df):
    folder = get_local_folder(self, project_handle, folder_name)
    f = io.BytesIO()
    df.to_parquet(f, compression="snappy", engine='pyarrow', index=False)
    f.seek(0)
    content = f.read()
...
def write_remote_folder_output(self, client, path, df):
    ...
    df.to_parquet("/tmp/temp.parquet", compression="snappy", engine='pyarrow', index=False)
    r = folder.put_file(path, "/tmp/temp.parquet")
    return